...
print.cumulcalib <- function(x, ...) {
  ...
  writeLines(sprintf(
    "  Mean calibration error (C_n): %.4g   |   C*: %.4g",
    x$C_n,
    x$C_star
  ))
...
#'   statistics (mean calibration, distance/bridge statistic, p-values, and the
#'   location of maximum drift) for the selected method.
...
summary.cumulcalib <- function(object, method = NULL, ...) {
  ...
  loc <- object$by_method[[method]]$loc
  ...
  structure(
    list(
      ...
      loc_time = object$data[loc, 't'],
      loc_pred = object$data[loc, 'X'],
