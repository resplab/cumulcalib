...
#' Cumulative calibration assessment for treatment benefit predictors
#'
#' This is the core function for performing cumulative calibration assessment for models for treatment benefit
#'
#' @return an objective of class cumulcalib that can be printed or plotted
#  @seealso [stringi::stri_length()] which this function wraps.
#' @param y vector of binary responses
...
plot.cumulcalib <- function(
  ...
  if (draw_sig) {
    ...
    if (method %in% c("BM2p")) {
      sig_p2 <- qMAD_BM_c(
        sig_level[2],
        w1 = unname(x$by_method$BM2p$stat_by_component['mean'])
      )
    }
  ...
  if (is.na(i)) {
    args$ylim <- c(
      min(W, -sig_p1 * (sign_p1 == -1), -sig_p2 * (sign_p2 == -1), -1),
      max(W, sig_p1 * (sign_p1 == 1), sig_p2 * (sign_p2 == 1), 1)
    )
  }
  ...
  if (method %in% c("BM2p", "BB")) {
    if (draw_stat) {
      lines(c(1, 1), c(0, W[n]), col = stat_col[1])
    }
    if (draw_sig) {
      lines(
        c(0, 1),
        c(sign_p1 * sig_p1, sign_p1 * sig_p1),
        col = stat_col[1],
        lty = 3
      )
    }
  ...
  #P2 lines
  if (method %in% c('BB')) {
    #If 2p bridge test then adjust the length of the red line and draw the bridge line
    lines(c(0, 1), c(0, W[n]), col = "gray", lty = 2)
    if (draw_stat) {
      lines(
        c(t_[loc], t_[loc]),
        c(t_[loc] / t_[n] * W[n], W[loc]),
        col = stat_col[2]
      )
    }
    if (draw_sig) {
      lines(
        c(0, 1),
        c(sign_p2 * sig_p2, sign_p2 * sig_p2 + W[n]),
        col = stat_col[2],
        lty = 3
      )
    }
    ...
    #BM or BM2p
    if (draw_stat) {
      lines(c(t_[loc], t_[loc]), c(0, W[loc]), col = stat_col[2])
    }
    if (draw_sig) {
      lines(
        c(0, 1),
        c(sign_p2 * sig_p2, sign_p2 * sig_p2),
        col = stat_col[2],
        lty = 3
      )
    }
